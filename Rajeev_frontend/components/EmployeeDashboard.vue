...
//     return blogs.value.filter(blog => (blog.status || 'Private') === 'published')
//   }
//   else if (filterStatus.value === 'private') {
//     return blogs.value.filter(blog => (blog.status || 'Draft') === 'Draft')
//   }
//   return blogs.value.filter(blog => (blog.status || 'Private') === filterStatus.value)
// })
...
async function fetchBlogs() {
  ...
  try {
    ...
    // Only keep blogs created by the logged-in user
    blogs.value = allBlogs.filter(
      blog => Number(blog.author) === Number(auth.user?.id)
    )
    console.log('Fetched blogs of current usser:', blogs)
  } catch (e) { 
